...
## 고려한 대안
- **수동 불리언 조합**(`isFormValid && isVerified && termsOk`) — 직관적이나 조건이 늘 때마다 게이트 식을 수정해야 하고,
  상태가 분산돼 누락·불일치 위험.
...
- 포기: `phoneVerified` 처럼 "값이 아닌 것"을 폼 필드로 모델링하는 약간의 개념적 우회. 단,
  인증 상태머신 자체는 `usePhoneVerification` 이 갖고 폼엔 **결과만** 반영해 책임을 분리했다(→ [0005](0005-phone-verification-state-machine.md)).

## 영향
- `src/signup/{schema.ts,SignupForm.tsx}`, `src/signup/phone/PhoneVerificationField.tsx`, `src/signup/TermsAgreement.tsx`.
