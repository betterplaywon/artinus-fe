...
## 새 입력 항목/약관이 필요할 때
- **새 필드**(5곳): `src/services/types.ts`(FieldKey) + `src/signup/schema.ts`(FIELD_SCHEMAS) + `src/signup/fieldRegistry.ts`(FIELD_COMPONENTS) + `src/signup/schema.ts`(SignupFormValues 키 + DEFAULT_VALUES 기본값). 컴포넌트는 `src/signup/fields.tsx`(또는 `phone/`)에 추가. 뒤 2곳을 빠뜨리면 컴파일 에러/controlled 경고로 드러난다.
- **새 약관**(4곳): `src/services/types.ts`(TermKey) + `src/signup/terms.ts`(TERM_DEFS) + `src/signup/schema.ts`(SignupFormValues 키 + DEFAULT_VALUES `false`).
